...
- 2024-07-26: **🧠 분석 계획 프롬프트 최종 고도화**: `PLANNING_PROMPT`의 지시문과 예시를 최종 수정하여, LLM이 사전에 제공된 데이터 컨텍스트를 활용해 불필요한 확인 단계를 생략하고 바로 처리 단계를 계획하도록 유도함. 이로써 Agent가 더욱 지능적이고 효율적인 분석 계획을 생성하게 됨.
- 2024-07-26: **🚑 자가 수정 안정성 최종 강화**: `main.py`의 자가 수정 블록에 `try-except` 예외 처리 구문을 추가하여, 수정된 코드의 재실행이 실패하더라도 전체 시스템이 중단되지 않고 해당 단계를 '최종 실패'로 명확히 기록한 뒤 다음 분석을 계속하도록 수정함. 이로써 시스템의 강건성을 확보함.
- 2024-07-26: **🧹 분석 계획 파싱 로직 강화**: `agent.py`의 `generate_analysis_plan` 메서드에 정규표현식을 이용한 파싱 로직을 추가하여, LLM이 생성하는 계획의 번호 형식이 어떻든 순수한 텍스트만 추출하도록 수정함. 이를 통해 오염된 계획으로 인한 후속 에러를 원천 차단함.
- 2024-07-26: **🧱 프롬프트 아키텍처 재설계**: `KeyError`를 원천 차단하기 위해 `system_prompts.py`를 `PROMPT`와 `EXAMPLES`로 분리하고, `agent.py`가 `.format()` 대신 `str.replace()`를 사용하도록 수정. 또한, `###JSON_START/END###` 구분자를 도입하고 `agent.py`에 다층 파싱 로직을 구현하여 LLM 응답 처리의 안정성을 대폭 향상시킴.
- 2024-07-26: **🧠 Agent 기억력 주입**: `CODE_GENERATION_PROMPT`에 `conversation_history`를 컨텍스트로 추가하고, `agent.py`가 이를 주입하도록 수정함. 이를 통해 LLM이 이전 단계의 실행 결과를 참조하여 다음 단계의 코드를 생성하는, 진정한 의미의 '상태 기반(Stateful)' 에이전트로 동작하도록 개선함.
- 2024-07-26: **🏗️ 상태 관리 아키텍처 혁신**: `[PREP]` 태그와 `data_profiler`를 도입하고, `main.py`가 이를 기반으로 조건부 상태 업데이트를 수행하도록 수정함. 또한 `context.py`, `agent.py`, `system_prompts.py`를 모두 변경된 아키텍처에 맞게 업데이트하여 시스템의 안정성과 지능을 근본적으로 개선함.
- 2024-07-26: **🔄 동적 컨텍스트 업데이트 구현**: `[PREP]` 단계 실행으로 데이터가 변경될 때마다 `data_profiler`를 재호출하여 `Context`의 데이터 요약을 갱신하도록 `main.py`를 수정함. 또한 `agent.py`와 관련 프롬프트를 수정하여, Agent가 매 단계마다 항상 최신 데이터 상태를 참조하며 코드를 생성하도록 아키텍처를 개선함.
- 2024-07-26: **📚 프롬프트 예시 복원 및 고도화**: `PLANNING_PROMPT`에 이전에 삭제되었던 다양한 통계 분석 예시를 모두 복원하고, `[PREP]` 태그 규칙과 '불필요한 단계 생략' 지침을 일관되게 적용하여 LLM의 계획 수립 능력을 최종적으로 고도화함.
- 2024-07-26: **🎨 프롬프트 예시 다양화**: `PLANNING_PROMPT`의 일부 예시에서 의도적으로 `[PREP]` 단계를 제거하여, LLM이 데이터 컨텍스트에 따라 불필요한 전처리 단계를 생성하지 않는 행동을 학습하도록 유도함.
- 2024-07-26: **📜 프롬프트 지침 명확화**: `PLANNING_PROMPT`의 예시에서 '필요하다면 변환'과 같은 모호한 표현을 '타입 변환'과 같은 명확한 '결정'의 표현으로 수정하여, Agent가 제공된 컨텍스트를 기반으로 더 확신을 갖고 계획을 수립하도록 개선함.
- 2024-07-26: **🛡️ 데이터 분할 시 상태 보존 규칙 추가**: `CODE_GENERATION_PROMPT`에 데이터 분할(`split`) 단계에서는 `df_result`에 분할된 일부가 아닌 원본 데이터프레임을 할당하도록 예외 규칙을 추가하여, 후속 단계에서 다른 그룹의 데이터가 유실되는 문제를 원천 차단함.
- 2024-07-26: **🐛 KeyError 해결 및 프롬프트 재설계**: `str.format()`으로 인한 `KeyError`를 해결하기 위해, `agent.py`가 안전한 `str.replace()`를 사용하도록 수정하고, `system_prompts.py`를 `PROMPT`와 `EXAMPLES`로 분리하여 프롬프트 포매팅의 안정성을 확보함.
